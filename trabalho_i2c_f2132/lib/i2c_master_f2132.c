...
volatile struct i2c_status_t i2c_status;
...
void init_i2c_master_mode()
{
    ...
    UCB0I2CSA = 0x01; //Dummy Slave Address
...
#endif
{
  if (IFG2 & UCB0RXIFG)                 // Receive Data Interrupt
  {
      ...
      if (i2c_status.rx_byte_count == 1) {
          ...
          //__bic_SR_register_on_exit(GIE);                // Enable interrupts: Use no Proteus
      ...
      switch (i2c_status.state)
      {
          ...
          case TX_DATA_MODE:
              if (i2c_status.tx_byte_count) {
                  ...
                  IE2 &= ~UCB0TXIE;                       // disable TX interrupt
                  //__bic_SR_register_on_exit(CPUOFF);      // Exit LPM0
                  //__bis_SR_register_on_exit(GIE);                // Enable interrupts: Use no Proteus
